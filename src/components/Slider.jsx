...
const Slider = () => {
  ...
  return (
    ...
    <div className="relative h-screen">
      ...
      <svg onClick={() => setIsOpen(!isOpen)} className={`fixed top-28 z-50 w-10 h-10 bg-black opacity-90 cursor-pointer p-2 rounded-full transition-all duration-300 ${isOpen ? 'left-2' : 'left-75'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white"><path d="M12 11V8L16 12L12 16V13H8V11H12Z" /><circle cx="12" cy="12" r="10" fill="none" stroke="white" strokeWidth="2" /></svg>
    
        <div className={`fixed top-0 left-0 h-full mt-[64px] overflow-y-scroll bg-black/60 border-b border-white/40 border-r border-white/40 transition-all duration-300 ease-in-out  ${isOpen ? 'w-0' : 'w-80 overflow-hidden'}`}>
