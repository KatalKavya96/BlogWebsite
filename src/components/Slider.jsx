...
const Slider = ({ setIsAuthenticated }) => {
  ...
  return (
    ...
    <div className="relative h-screen ">
      ...
      <svg onClick={() => setIsOpen(!isOpen)} className={`fixed top-28 z-50 w-9 h-9 bg-black/70 cursor-pointer p-1 rounded-full transition-all duration-300 ${isOpen ? 'left-2' : 'left-85.5'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white"><path d="M12 11V8L16 12L12 16V13H8V11H12Z" /><circle cx="12" cy="12" r="10" fill="none" stroke="white" strokeWidth="2" /></svg>
        ...
        <div className={`fixed top-0 left-0 h-[70vh] mt-[64px] overflow-y-scroll  border-b border-white/20 border-r border-white/20 transition-all duration-300 ease-in-out bg-adder shadow-[5px_10px_15px_2px_rgba(0,0,0,0.5)]    ${isOpen ? 'w-0' : 'w-90 overflow-hidden custom-scroll-hide'}`}>
          ...
          <div className="flex flex-col text-white gap-6 p-6">
            ...
            <div className='w-90 h-25 border-t border-t-white/20 border-r border-r-white/20 fixed bottom-0 left-0 flex justify-between items-center p-[20px] shadow-[5px_10px_15px_2px_rgba(0,0,0,0.5)]'>
              ...
              <div className='relative h-8 w-8 text-white cursor-pointer'>
                <svg onClick={() => setShowMenu(!showMenu)} className='opacity-80 cursor-pointer' fill="white" viewBox="0 0 24 24">
                  <g><path d="M3 12c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm9 2c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm7 0c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2z" /></g>
                </svg>

                {showMenu && (
                  <div className="absolute -bottom-7 -left-5 bg-black/10 text-white border border-white/20 rounded-lg shadow-lg px-2 py-1 z-50">
                    <button
                      onClick={() => {
